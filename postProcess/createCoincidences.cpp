...
int main(int argc, char **argv) {
  ...
  double energyWindow;         // %
  double promptGammaThreshold; // KeV
  ...
  // Prompt gamma threshold (KeV)
  infoS >> promptGammaThreshold;
  infoS.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

  // Prompt gamma max (KeV)
  infoS >> promptGammaMax;
  infoS.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  ...
  case COINCIDENCE_METHOD::TAKE_CLOSEST_MULTIPLEXING:
    coinMethod = COINCIDENCE_METHOD::TAKE_CLOSEST_MULTIPLEXING;
    break;
  default:
    coinMethod = COINCIDENCE_METHOD::TAKE_CLOSEST;
  }
  ...
  if (coinMethod == COINCIDENCE_METHOD::TAKE_CLOSEST_MULTIPLEXING) {
    ...
    if (fLM_AB == nullptr) {
      printf("Unable to create LM file 'data_AB.lm'\n");
      return -1;
    }
    fwrite(&header, sizeof(LMHeader), 1, fLM_AB);

    // Use data_B_prime.lm
    fLM_B_prime = fopen("data_B_prime.lm", "wb");
    if (fLM_B_prime == nullptr) {
      printf("Unable to create LM file 'data_B_prime.lm'\n");
      return -1;
    }
  ...
  int errRead =
      firstSingleIt->readPenRed(fmods[firstSingleMod], firstSingleMod, emin,
                                readingUpperLimit, eRes, normDist, gen);
  if (errRead != 0) {
    if (errRead == 1) {
      // End of file
  ...
  while (true) {
    ...
    bool isTriple = false; // Flag for Multiplexing
    ...
    if (iterations % 100000 == 0) {
      ...
      for (size_t im = 0; im < fmods.size(); ++im) {
        char prefix[100];
        snprintf(prefix, 100, " Module %4d", (int)(im + 1));
        progressSstring +=
