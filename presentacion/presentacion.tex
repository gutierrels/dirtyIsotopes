...
\begin{frame}{Flujo de Trabajo Metodológico}
    ...
    \begin{tikzpicture}[
        ...
        \node (post) [box, right=of sim] {\includegraphics[height=0.9cm]{cpp_logo.png}\\[1mm] 3. Post-procesado\\(C++: singles a\\coincidencias LM)};
    ...
    \begin{columns}[T]
        \begin{column}{0.54\textwidth}
            \begin{block}{Conclusiones Clave}
                \begin{itemize}\setlength{\itemsep}{0.6em}
                    \item \textbf{Validación Monte Carlo:} Entorno preciso y fiel con \penred y Blender.
                    \item \textbf{Física del Ruido:} El scatter Compton de prompt gammas es el principal origen del ruido.
                    \item \textbf{Optimización Física:} Contraste aumentado un 79\% estrechando la ventana EW al 10\%.
                    \item \textbf{Multiplexado mPET:} Separación de trazadores viable mediante coincidencias triples.
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.41\textwidth}
            \begin{exampleblock}{Transferencia y Relevancia}
                \centering
                \small
                \vspace{0.1cm}
                \textbf{Colaboración con Bruker} para la futura optimización comercial del mPET.\par
                \vspace{0.25cm}
                \hrule
                \vspace{0.25cm}
                Aceptado para comunicación en el:\par
                \textbf{European Congress of Medical Physics}\par
                \vspace{0.25cm}
                \includegraphics[width=0.65\textwidth]{logo_congreso.png}\par
                \vspace{0.05cm}
                {\tiny\color{mcmBlack!50} 6th ECMP (Valencia, 2026)\par}
            \end{exampleblock}
        \end{column}
    \end{columns}
    % NOTA DEL PONENTE: Como conclusiones finales, quiero destacar el valor de transferencia y la relevancia internacional de este trabajo. A nivel científico, validamos el entorno de simulación, identificamos la física del ruido Compton y demostramos que estrechar la ventana al 10% mejora el contraste un 79%. A nivel de empresa y transferencia tecnológica, este proyecto abre camino en colaboración con Bruker para comercializar el multiplexado. Y finalmente, como prueba de su alta relevancia, el trabajo ha sido aceptado para su presentación en el próximo Congreso Europeo de Física Médica, el ECMP de 2026, lo cual consolida el valor y el rigor de los resultados obtenidos.
\end{frame}
...
% --- Diapositiva 25: Presupuesto ---
\begin{frame}{Presupuesto Estimado}
    \begin{columns}[c]
        \begin{column}{0.52\textwidth}
            \begin{block}{Desglose de Costes}
                Resumen de las partidas presupuestarias del proyecto:
                \begin{itemize}\setlength{\itemsep}{0.5em}
                    \item \textbf{Personal:} Dedicación del autor y tutorización del proyecto.
                    \item \textbf{Software:} Amortización de la licencia de reconstrucción de imagen Bruker.
                    \item \textbf{Hardware:} Uso del clúster de computación QUASAR y equipo local.
                \end{itemize}
            \end{block}
        \end{column}
        ...
        \end{column}
    \end{columns}
    % NOTA DEL PONENTE: Respecto al presupuesto estimado para el desarrollo del proyecto, este se divide en tres áreas principales: costes de personal por valor de 8110 euros, costes de software por 8000 euros asociados a la amortización proporcional de la licencia de Bruker RecoServer, y costes de hardware por 527 euros correspondientes a la amortización del portátil y el uso del clúster QUASAR. Con los gastos generales del 10% y el IVA de 21%, el importe total del presupuesto asciende a 22144,73 euros.
\end{frame}
