...
\begin{frame}{El Paradigma de la Teranosis}
    \begin{block}{El Vehículo y la Carga Radiológica}
        \begin{itemize}
            \item \alert{Terapia + diagnóstico:} \textit{«Vemos lo que tratamos y tratamos lo que vemos»}.
            \item \textbf{Molécula portadora:} Vector biológico que reconoce y se une selectivamente a los receptores tumorales.
            \item \textbf{Isótopos (Carga):} Se acoplan a la molécula en fases clínicas distintas.
        \end{itemize}
    \end{block}
    \begin{exampleblock}{El Par Teranóstico Verdadero: \yochoseis / \ynuevecero}
        \begin{itemize}
            \item Al ser el mismo elemento, el cuerpo no los distingue (\textbf{biodistribución 100\% idéntica}).
            \item \textbf{\yochoseis:} Emisor $\beta^+$ para el escáner PET previo (planificación).
            \item \textbf{\ynuevecero:} Emisor $\beta^-$ para radioterapia dirigida (destrucción).
        \end{itemize}
    \end{exampleblock}
    \vspace{0.2cm}
    % NOTA DEL PONENTE: Buenos días. Comenzamos la presentación de mi TFG. La teranosis en medicina nuclear funciona uniendo un isótopo a una molécula portadora, un vector biológico con alta afinidad por los receptores del tumor. La clave está en la carga. El par Y-86/Y-90 es lo que llamamos un par teranóstico verdadero. Como son el mismo elemento, la química es idéntica y el cuerpo no los distingue. Primero inyectamos la molécula con Y-86, el "explorador" emisor de positrones, para hacer un PET y planificar. Días después, inyectamos la misma molécula con Y-90, el "tratamiento", para hacer radioterapia local. El Y-86 nos da un mapa 100% perfecto de dónde irá la radioterapia.
\end{frame}
...
\begin{frame}{Motivación Clínica: Multiplexado (mPET)}
    \begin{columns}[c]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 4. Esquema de adquisición simultánea dual (mPET).\par}
            \vspace{0.2cm}
...
% --- Diapositiva 7 (1/2) ---
\begin{frame}{Alcance y Objetivos del Proyecto (1/2)}
    ...
    \begin{exampleblock}{Alcance Específico}
        \begin{itemize}
            \item \textbf{Modelado y Simulación:} Geometría del escáner Bruker Si78 y fantoma Derenzo en Blender, simulados en \penred{}.
            \item \textbf{Software de Post-procesado:} Algoritmos en C++ para extracción de coincidencias y clasificación de eventos triples.
        \end{itemize}
    \end{exampleblock}
    % NOTA DEL PONENTE: El alcance del proyecto, tal y como se detalla en la memoria, abarca 4 fases fundamentales. En esta primera parte abordamos: 1) El modelado y simulación computacional: modelamos el escáner comercial Bruker y el fantoma Derenzo en Blender, simulando la física de desintegración con el framework penRed y su módulo PENNUC. 2) Desarrollo de software propio en C++ para procesar los datos list mode y aislar las coincidencias triples, basándonos en Andreyev y Celler.
\end{frame}

% --- Diapositiva 7 (2/2) ---
\begin{frame}{Alcance y Objetivos del Proyecto (2/2)}
    \begin{exampleblock}{Alcance Específico}
        \begin{itemize}
            \item \textbf{Estudio de Optimización:} Barrido de ventanas (EW y CW) evaluando métricas de imagen (CRC y SNR) sobre reconstrucciones MLEM.
            \item \textbf{Prueba de Concepto (mPET):} Simulación de fantoma dual (\yochoseis/\fdiezocho) y separación mediante sustracción de imagen.
        \end{itemize}
    \end{exampleblock}
    % NOTA DEL PONENTE: Siguiendo con los objetivos, la segunda parte incluye: 3) Un estudio exhaustivo de optimización paramétrica donde hicimos un barrido de las ventanas de energía y coincidencia para maximizar el contraste y SNR de las reconstrucciones MLEM. 4) Y finalmente, aplicar todo esto en una prueba de concepto de imagen multiplexada, simulando dos isótopos simultáneamente para demostrar la viabilidad de su separación física y computacional.
\end{frame}
...
\begin{frame}{Flujo de Trabajo Metodológico}
    ...
    \begin{tikzpicture}[
        ...
        \node (blender) [box] {\includegraphics[height=0.9cm]{logo_blender.png}\\[1mm] 1. Modelado 3D\\(Blender + plugin\\\penred)};
        \node (sim) [box, right=of blender] {\includegraphics[height=0.9cm]{PenRed_color.png}\\[1mm] 2. Simulación MC\\(Monte Carlo\\\penred + PENNUC)};
        ...
        \node (reco) [box, below=of post, yshift=-0.5cm] {\includegraphics[height=0.6cm]{logo_bruker.pdf}\\[1mm] 4. Reconstrucción\\(Bruker RecoServer\\algoritmo MLEM)};
...
\begin{frame}{Modelado Geométrico: Escáner y Fantoma}
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 5. Modelado geométrico tridimensional del escáner Bruker Si78 y el fantoma Derenzo en Blender.\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Simulación Monte Carlo: Definiciones Clave}
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 6. Visualización del transporte de trazas de partículas secundarias simuladas en \penred.\par}
            \vskip0.2cm
...
\begin{frame}{Desarrollo en C++: Post-procesado}
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 7. Definición espacial y geométrica de materiales y componentes del escáner en C++.\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Validación del Espectro del Itrio-86}
    \begin{columns}[T]
        ...
        \begin{column}{0.58\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 8. Espectro simulado de depósito de energía de singles de \yochoseis en vacío.\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Simulación Ideal (Fase 1: Ground Truth)}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 9. Reconstrucción tomográfica ideal MLEM (Fase 1: Ground Truth).\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Caracterización de la Degradación (Fase 2)}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 10. Imagen reconstruida de \yochoseis bajo el efecto de scatter y atenuación en agua (Fase 2).\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Caracterización de la Degradación (Fase 3)}
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 11. Espectro realista de energía final registrado en los detectores (Fase 3).\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Comparativa Crítica: \yochoseis vs \fdiezocho}
    ...
    \vskip0.05cm
    {\scriptsize\color{mcmBlack!65} Figura 12. Comparativa de reconstrucciones en condiciones reales: \fdiezocho (izquierda) e \yochoseis (derecha) bajo idénticos parámetros de adquisición.\par}
...
\begin{frame}{Optimización: Ventana de Energía (EW)}
    ...
    {\scriptsize\color{mcmBlack!65} Figura 13. Efecto visual de la reducción de la ventana de energía EW (20\% izquierda, 15\% centro, 10\% derecha).\par}
...
\begin{frame}{Optimización: Ventana CW y Trade-off}
    \begin{columns}[T]
        ...
        \begin{column}{0.46\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 14. Curvas de compromiso entre contraste (CRC) y ruido (SNR) en función de la ventana EW.\par}
        \end{column}
    \end{columns}
...
\begin{frame}{Prueba de Concepto mPET: Metodología}
    \begin{columns}[T]
        ...
        \begin{column}{0.42\textwidth}
            ...
            \vskip0.1cm
            {\scriptsize\color{mcmBlack!65} Figura 15. Geometría del fantoma cilindro dual simulado para la adquisición mPET simultánea.\par}
        \end{column}
    \end{columns}
    \vspace{0.2cm}
    \hfill {\tiny\color{mcmBlack!40} [Pratt et al., 2023]}
...
\begin{frame}{Prueba de Concepto mPET: Resultados}
    ...
    \begin{center}
        ...
        \vskip0.1cm
        {\scriptsize\color{mcmBlack!65} Figura 16. Distribuciones de actividad en multiplexado: mixta (AB), Y-86 aislado con triples ($B'$) y F-18 puro recuperado ($A$).\par}
    \end{center}
