...
\begin{figure}
  ...
  \caption{Ejemplo de la trayectoria de electrones en un medio acuoso. El color muestra el cambio en la energía del electrón. Fuente: Manual de Usuario de PenRed \cite{gimenez2025penred} (Reproducida con permiso de los autores).}
...
El modelo virtual consta de tres anillos de detectores con las siguientes características:
...
\begin{lstlisting}[language=C++, caption={Implementación de la lógica de detección basada en tomar el primer evento válido dentro de la ventana de coincidencia.}, label={lst:take_closest_code}]
...
\begin{itemize}
  \item \textbf{Configuración de Materiales:} Se definen todos los volúmenes del fantoma (varillas y cuerpo) y del entorno como vacío (\texttt{void.mat}). Esto elimina por completo la atenuación y la dispersión Compton (\textit{scatter}). Los fotones viajan en línea recta desde la emisión hasta el detector. Adicionalmente, dado que en el vacío el positrón emitido no encontraría electrones con los que interactuar, se ha forzado en la simulación su aniquilación inmediata en el mismo punto de la desintegración nuclear. Esta aproximación metodológica tiene la ventaja añadida de eliminar también el efecto de degradación de imagen asociado al rango del positrón.
...
El post-procesado se ejecutó en modo \textbf{\textit{Take Closest Multiplexing}} con una ventana de energía del 10\% [\SI{460}{\kilo\electronvolt}, \SI{562}{\kilo\electronvolt}], una ventana de coincidencia de \SI{4}{\nano\second} y un umbral de detección de \textit{prompt gamma} de \SI{650}{\kilo\electronvolt}.
...
\begin{figure}[htbp]
  ...
  \caption{Asignación de materiales en la geometría de simulación para la prueba de concepto de multiplexado (Fase 5). Los colores (cuadrados) se corresponden con la visualización gráfica de la derecha. Todas las regiones emplean materiales realistas (agua, aire, LYSO).}
...
\begin{enumerate}
  ...
  \item \textbf{Sustracción de Imágenes:} Se aplicó un post-procesado en Python \ref{lst:python_post} para recuperar la imagen del isótopo puro ($^{18}\text{F}$) sustraendo la contaminación estimada de la imagen total:
        ...
        \begin{equation}
          I_{\text{F18}} = I_{\text{total}} - F \cdot I_{\text{Y86}}^{\text{T}}
          \label{eq:sustraccion}
        \end{equation}

        El factor $F$ se determinó experimentalmente comparando la intensidad en las regiones conocidas de solo $^{86}\text{Y}$ en ambas imágenes.
\end{enumerate}
