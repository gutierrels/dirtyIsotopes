...
Específicamente, para la configuración de referencia utilizada en este estudio, se empleó el algoritmo MLEM con 16 iteraciones y 1 único subset. El volumen de reconstrucción se definió como una matriz de $180 \times 180 \times 300$ vóxeles, con un tamaño de vóxel isotrópico de \SI{0.50}{\milli\meter}. Esta discretización espacial permite una resolución suficiente para resolver las varillas más finas del fantoma Derenzo sin comprometer excesivamente la estadística por vóxel.

Es importante destacar que, para aislar los efectos de las estrategias de post-procesado propuestas en este trabajo (ventanas de energía y tiempo), se desactivaron las correcciones estándar del software de reconstrucción: Scatter, Randoms y Atenuación se establecieron a 0 o \enquote{None}. Únicamente se mantuvo activo el modelado de la respuesta del sistema (\textit{Point Spread Function}) con un valor de PSF = 1.0. Estos parámetros de reconstrucción —tanto el algoritmo MLEM como configuración de filtros— se mantuvieron constantes para todas las fases experimentales (desde la ideal hasta la optimizada). De este modo, cualquier variación en la calidad de imagen final es atribuible exclusivamente a los cambios en los parámetros del post-procesado (\textit{input data}) y no al algoritmo de reconstrucción.
...
\begin{figure}[hbtp]
  ...
  \caption{Configuración del fantoma para el estudio de multiplexado.}
