...
Para reconstruir imágenes PET, es necesario procesar estos eventos individuales para identificar pares de fotones que correspondan a una aniquilación. Dado que este proceso requiere una lógica específica no disponible en las herramientas de reconstrucción estándar, se ha utilizado un software de post-procesado escrito en \textbf{C++}

La base del código de post-procesado (\texttt{createCoincidences}) es una herramienta de investigación interna desarrollada en \textbf{Bruker}. Sobre esta arquitectura base, en el marco de este TFG, se han desarrollado e implementado extensiones específicas para incorporar la lógica de detección de triples y separación de isótopos (Multiplexing), adaptando la metodología descrita en la literatura \parencite{Andreyev2011}.
...
\begin{enumerate}
	...
	\item \textbf{Modelado de la Respuesta del Detector (\textit{Energy Blurring}):}
	      ...
	      \end{equation}
	      y se suma un valor aleatorio de una distribución normal $\mathcal{N}(0, \sigma)$ a la energía original. Este paso es fundamental para simular la respuesta real del cristal centelleador y evaluar el rechazo de eventos dispersos (\textit{scatter}).

	\item \textbf{Salida:}
	      Los eventos de coincidencia identificados se proyectan sobre la superficie de los detectores y se escriben en formato List-Mode (LM), soportando estándares como Bruker LM y CASToR.
\end{enumerate}
...
\begin{itemize}
	\item \textbf{Configuración de Materiales:} Se definen todos los volúmenes del fantoma (varillas y cuerpo) y del entorno como \textbf{vacío} (\texttt{void.mat}). Esto elimina por completo la \textbf{atenuación} y la \textbf{dispersión Compton (\textit{scatter})}. Los fotones viajan en línea recta desde la emisión hasta el detector.
...
Se simulan un total de \SI{2e8} historias en un tiempo de adquisición de 2 horas lo que resulta en una actividad de \SI{27.77}{\kilo\becquerel}.
...
\begin{itemize}
    \item \textbf{Configuración de Simulación:} Se utilizan \textbf{exactamente los mismos archivos de datos} generados en la Fase 2 (misma configuración de materiales y fuentes). No es necesario realizar una nueva simulación Monte Carlo.
...
\begin{enumerate}
    \item \textbf{Ventana de Energía (EW):} El objetivo es rechazar los fotones que han sufrido \textit{scatter} y, específicamente para el $^{86}\text{Y}$, filtrar los \textit{prompt gammas} de menor energía (como el de \SI{628}{\kilo\electronvolt}) que contaminan la ventana estándar. Se definieron tres ventanas centradas en el fotolípico de \SI{511}{\kilo\electronvolt}, progresivamente más estrechas:
    \begin{itemize}
        \item \textbf{EW 20\%:} [\SI{409}{\kilo\electronvolt}, \SI{613}{\kilo\electronvolt}]. Ventana estándar comercial, utilizada como referencia.
        \item \textbf{EW 15\%:} [\SI{434}{\kilo\electronvolt}, \SI{588}{\kilo\electronvolt}]. Ventana intermedia.
        \item \textbf{EW 10\%:} [\SI{460}{\kilo\electronvolt}, \SI{562}{\kilo\electronvolt}]. Ventana muy restrictiva, diseñada para maximizar el rechazo de eventos espurios a costa de sensibilidad.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{CW = \SI{4}{\nano\second}:} Valor estándar ajustado para detectores LYSO rápidos.
        \item \textbf{CW = \SI{3}{\nano\second}:} Valor agresivo para minimizar la ventana temporal de integración de ruido.
    \end{itemize}
\end{enumerate}

\textbf{Simulación de Alta Estadística (Convergencia):}
Una vez identificados los parámetros óptimos mediante el barrido anterior (realizado con $1.5\cdot10^9$ historias), se ejecutó una simulación final (\texttt{SIM\_FINAL}) con una estadística masiva de $6\cdot10^9$ historias. El objetivo de esta simulación es obtener una imagen con el mínimo ruido estadístico posible para validar que el contraste recuperado es real y robusto, y no un artefacto de las fluctuaciones estadísticas del fondo.
...
\begin{itemize}
    \item \textbf{Región A:} Varillas llenas exclusivamente con $^{18}\text{F}$ (isótopo puro).
    \item \textbf{Región B:} Varillas llenas exclusivamente con $^{86}\text{Y}$ (isótopo sucio).
\end{itemize}
...
\begin{enumerate}
    \setcounter{enumi}{1} % Para que empiece en el número 2 si continúas una lista previa
    ...
    \item \textbf{Sustracción de Imágenes:} Se aplicó un post-procesado en Python para recuperar la imagen del isótopo puro ($^{18}\text{F}$) sustraendo la contaminación estimada de la imagen total:
...
\subsection*{Asignacion de materiales }
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[c]{0.38\textwidth}
        \centering
        % Asegúrate de tener la captura de pantalla con estos colores
